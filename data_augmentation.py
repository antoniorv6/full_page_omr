...
def augment(image, force_one_channel=False):
    ...
    transform = transforms.Compose(
        [   
            ...
            transforms.RandomApply([SignFlipping()], p=0.2),
        ]
    )
    ...
    return transforms.ToTensor()(image).detach().numpy()

def convert_to_tensor_format(x):
    transformations = transforms.Compose([
        transforms.ToPILImage(),
        transforms.ToTensor()
    ])
    return transformations(x).detach().numpy()